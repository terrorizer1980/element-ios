...
class HomeserverAddress {
    ...
    static func sanitize(address: String) -> String {
        return !address.contains("://") ? "https://\(address.lowercased())" : address.lowercased()
    }
    
    /// Strips the `https://` away from the address (but leaves `http://`) for display in labels.
    static func displayable(address: String) -> String {
        return address.replacingOccurrences(of: "https://", with: "")
    }
}
